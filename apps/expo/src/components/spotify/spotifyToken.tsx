...
async function tokensInit(code: string) {
  ...
  try {
    ...
    // console.log(responseJson, 'my brother')
...
async function getTrackData() {
  ...
  if (!(access === null || access === undefined)) {
    ...
    console.log('Access token ye yeye: ', access)
...
export async function updateSpotifyData(userID: number) {
  ...
  if (returnedTrack.userID === -1) {
    ...
    // Bad data
    console.log(
      "Bad data retrieved or no data retrieved, thus returned default values. Don't update database",
    )
  }
