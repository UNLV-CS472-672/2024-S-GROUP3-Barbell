...
export default function MessageView() {
  ...
  const {
    ...
    isFetching,
    isFetched,
  } = api.notif.getDMsFromChatId.useQuery({
            ...
            <View className="flex gap-y-3">
              {messages?.map((message) =>
                message.senderId == user.id ? (
                  <View className="bg-dark-purple w-1/2 self-end rounded-xl px-4 py-3">
                    <Text className="text-left text-slate-200" key={message.id}>
                      {message.content + '\n'}
                    </Text>
                  </View>
                ) : (
                  <View className="w-1/2 self-start rounded-xl bg-zinc-800 px-4 py-3">
                    <Text className="text-left text-slate-200" key={message.id}>
                      {message.content + '\n'}
                    </Text>
                  </View>
